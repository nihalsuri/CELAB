...
tstp.end_staircase_p = friction_est.time*friction_est.num;
tstp.start_staircase_n = tstp.end_staircase_p + friction_est.time;

idx.staircase_p = (out_omega_motor.time <= tstp.end_staircase_p);
idx.staircase_n = (out_omega_motor.time >= tstp.start_staircase_n);
...
for i=1:friction_est.num
    ...
    lp.t_start = (i-1)*friction_est.time+1;
    lp.t_stop = (i)*friction_est.time-1;
    % Function handle to compute indices
    lp.get_idx = @(times) (times >= lp.t_start) .* (times <= lp.t_stop);
...
%{
plt.f1 = plot_meas(sOut.omeg.p, sOut.omeg.means(:,1), 1, friction_est, plt, "\omega_m [krpm]");
plt.f2 = plot_meas(sOut.omeg.n, sOut.omeg.means(:,2), 2, friction_est, plt, "\omega_m [krpm]");
plt.f3 = plot_meas(sOut.curr.p, sOut.curr.means(:,1), 3, friction_est, plt, "i_a [A]");
plt.f4 = plot_meas(sOut.curr.n, sOut.curr.means(:,2), 4, friction_est, plt, "i_a [A]");
%}
...
% Whole Phi matrix
plt.Phi_p = ls.Phi(sOut.curr.means(:,1));
...
plt.Phi_ = [plt.Phi_n(end,:);
            plt.Phi_n(1,:);
            plt.Phi_p];
% Calculate torque with averaged estiamtes
plt.tau_ = plt.Phi_*[ls.Beq; ls.tau_sf];
% Plot averaged torque for positive and negaticve values
plot(plt.Phi_(:,1), plt.tau_)
...
function [f] =  plot_meas(ts, means, fig, friction_est, plt, ylab)
