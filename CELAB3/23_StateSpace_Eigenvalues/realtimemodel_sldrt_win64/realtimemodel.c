...
void realtimemodel_output(void)
{
  ...
  /* Outputs for Enabled SubSystem: '<Root>/Feedback Controller' incorporates:
  ...
  /* End of Outputs for SubSystem: '<Root>/Feedback Controller' */
  if (tmp) {
  ...
  if (tmp_0) {
     ...
     */
    if (tmp) {
      if (realtimemodel_B.HiddenBuf_InsertedFor_FeedbackController_at_inport_2 >
          0.0) {
        if (!realtimemodel_DW.FeedbackController_MODE) {
          (void) memset(&(realtimemodel_XDis.Integrator1_CSTATE), 0,
                        1*sizeof(boolean_T));
          realtimemodel_DW.FeedbackController_MODE = true;
        }
      } else {
        if (realtimemodel_M->Timing.t[1] == rtmGetTStart(realtimemodel_M)) {
          (void) memset(&(realtimemodel_XDis.Integrator1_CSTATE), 1,
                        1*sizeof(boolean_T));
        }
  ...
  if (realtimemodel_DW.FeedbackController_MODE) {
    ...
    /* Gain: '<S2>/deg2rad_1' */
    rtb_Sum_idx_0 = realtimemodel_P.deg2rad * realtimemodel_B.th_hubdeg;

    /* Gain: '<S2>/Integral gain1' incorporates:
     *  Gain: '<S2>/intOn'
     *  Sum: '<S2>/Sum5'
     */
    realtimemodel_B.Integralgain1 = (rtb_Sum1 - rtb_Sum_idx_0) *
      realtimemodel_P.sIn.intOn * realtimemodel_P.feedback.robustKi;
     ...
     *  Sum: '<S2>/Sum'
     */
    rtb_Sum1 = (((((realtimemodel_P.feedback.Nx[0] * rtb_Sum1 - rtb_Sum_idx_0) *
                   realtimemodel_P.feedback.K[0] + (realtimemodel_P.feedback.Nx
      [1] * rtb_Sum1 - realtimemodel_P.deg2rad * realtimemodel_B.Sum1) *
                   realtimemodel_P.feedback.K[1]) +
                  (realtimemodel_P.feedback.Nx[2] * rtb_Sum1 -
                   realtimemodel_P.deg2rad * tmp_1[0]) *
                  realtimemodel_P.feedback.K[2]) + (realtimemodel_P.feedback.Nx
      [3] * rtb_Sum1 - realtimemodel_P.deg2rad * tmp_1[1]) *
                 realtimemodel_P.feedback.K[3]) + realtimemodel_P.feedback.Nu *
                rtb_Sum1) + realtimemodel_X.Integrator1_CSTATE;
...
void realtimemodel_derivatives(void)
{
  ...
  /* Derivatives for Enabled SubSystem: '<Root>/Feedback Controller' */
  if (realtimemodel_DW.FeedbackController_MODE) {
    /* Derivatives for Integrator: '<S2>/Integrator1' */
    _rtXdot->Integrator1_CSTATE = realtimemodel_B.Integralgain1;
  } else {
    ((XDot_realtimemodel_T *) realtimemodel_M->derivs)->Integrator1_CSTATE = 0.0;
  }

  /* End of Derivatives for SubSystem: '<Root>/Feedback Controller' */
}
...
void realtimemodel_initialize(void)
{
  ...
  /* Start for Enabled SubSystem: '<Root>/Feedback Controller' */
  realtimemodel_DW.FeedbackController_MODE = false;
  (void) memset(&(realtimemodel_XDis.Integrator1_CSTATE), 1,
                1*sizeof(boolean_T));

  /* End of Start for SubSystem: '<Root>/Feedback Controller' */
  ...
  /* SystemInitialize for Enabled SubSystem: '<Root>/Feedback Controller' */
  ...
  /* End of SystemInitialize for SubSystem: '<Root>/Feedback Controller' */
}
...
RT_MODEL_realtimemodel_T *realtimemodel(void)
{
  ...
  /* External mode info */
  realtimemodel_M->Sizes.checksums[0] = (431406746U);
  realtimemodel_M->Sizes.checksums[1] = (986516385U);
  realtimemodel_M->Sizes.checksums[2] = (1920903954U);
  realtimemodel_M->Sizes.checksums[3] = (3270832974U);
  ...
  realtimemodel_M->Sizes.numBlocks = (34);/* Number of blocks */
  realtimemodel_M->Sizes.numBlockIO = (10);/* Number of block outputs */
  realtimemodel_M->Sizes.numBlockPrms = (39);/* Sum of parameter "widths" */
