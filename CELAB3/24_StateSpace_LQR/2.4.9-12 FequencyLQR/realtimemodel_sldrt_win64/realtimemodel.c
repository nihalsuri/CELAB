...
void realtimemodel_output(void)
{
  ...
  /* Reset subsysRan breadcrumbs */
  srClearBC(realtimemodel_DW.FeedbackControllers_SubsysRanBC);
  ...
  /* S-Function Block: <S4>/Encoder Input */
  {
    ...
    RTBIO_DriverIO(0, ENCODERINPUT, IOREAD, 1,
                   &realtimemodel_P.EncoderInput_Channels, &rtb_Integrator,
                   &parm);
  }
  ...
  /* S-Function Block: <S4>/Analog Input */
  {
    ...
    RTBIO_DriverIO(0, ANALOGINPUT, IOREAD, 2,
                   realtimemodel_P.AnalogInput_Channels, &rtb_xQ[0], &parm);
  }

  /* Gain: '<Root>/Pulses2Deg' */
  realtimemodel_B.th_hubdeg = realtimemodel_P.sens.enc.pulse2deg *
    rtb_Integrator;

  /* Sum: '<S1>/Sum' */
  rtb_Integrator = rtb_xQ[0] - rtb_xQ[1];

  /* Gain: '<S1>/V2Deg1' */
  realtimemodel_B.th_ddeg = realtimemodel_P.sens.pot2.V2deg * rtb_Integrator;

  /* Clock: '<S5>/Clock' */
  rtb_Integrator = realtimemodel_M->Timing.t[0];

  /* Outputs for Enabled SubSystem: '<Root>/Feedback Controllers' incorporates:
  ...
  /* End of Outputs for SubSystem: '<Root>/Feedback Controllers' */
  if (tmp) {
     ...
     */
    realtimemodel_DW.Average_MODE = ((realtimemodel_P.sIn.t0 <= rtb_Integrator) &&
      (rtb_Integrator <= realtimemodel_P.sIn.t1));
  }
  ...
  if (tmp_0) {
    ...
    if ((realtimemodel_DW.clockTickCounter < realtimemodel_P.PulseGenerator_Duty)
        && (realtimemodel_DW.clockTickCounter >= 0)) {
      /* DiscretePulseGenerator: '<Root>/Pulse Generator' */
      realtimemodel_B.PulseGenerator = realtimemodel_P.PulseGenerator_Amp;
    } else {
      /* DiscretePulseGenerator: '<Root>/Pulse Generator' */
      realtimemodel_B.PulseGenerator = 0.0;
    }

    if (realtimemodel_DW.clockTickCounter >=
        realtimemodel_P.PulseGenerator_Period - 1.0) {
      realtimemodel_DW.clockTickCounter = 0;
    } else {
  ...
  if (tmp_0) {
     ...
     */
    if (tmp) {
      if (realtimemodel_B.HiddenBuf_InsertedFor_FeedbackControllers_at_inport_2 >
          0.0) {
        if (!realtimemodel_DW.FeedbackControllers_MODE) {
          (void) memset(&(realtimemodel_XDis.Integrator_CSTATE), 0,
                        3*sizeof(boolean_T));
          realtimemodel_DW.FeedbackControllers_MODE = true;
        }
      } else {
        ...
        if (realtimemodel_DW.FeedbackControllers_MODE) {
          (void) memset(&(realtimemodel_XDis.Integrator_CSTATE), 1,
                        3*sizeof(boolean_T));
          realtimemodel_DW.FeedbackControllers_MODE = false;
        }
      }
    }
  ...
  if (realtimemodel_DW.FeedbackControllers_MODE) {
    ...
    if (tmp_0) {
      /* Gain: '<S2>/deg2rad' */
      realtimemodel_B.deg2rad = realtimemodel_P.deg2rad *
        realtimemodel_B.PulseGenerator;

      /* Gain: '<S2>/Input feedforward gain (Nu)' */
      realtimemodel_B.InputfeedforwardgainNu = realtimemodel_P.feedback.Nu *
        realtimemodel_B.deg2rad;
    }

    /* Gain: '<S2>/deg2rad_1' */
    realtimemodel_B.x[0] = realtimemodel_P.deg2rad * realtimemodel_B.th_hubdeg;
    realtimemodel_B.x[1] = realtimemodel_P.deg2rad * realtimemodel_B.Sum1;
    realtimemodel_B.x[2] = realtimemodel_P.deg2rad * tmp_4[0];
    realtimemodel_B.x[3] = realtimemodel_P.deg2rad * tmp_4[1];

    /* Sum: '<S2>/Sum2' */
    rtb_Integrator = realtimemodel_B.deg2rad - realtimemodel_B.x[0];

    /* Gain: '<S2>/Integral gain' incorporates:
     *  Gain: '<S2>/Integrator Choice'
     */
    realtimemodel_B.Integralgain = realtimemodel_P.sIn.intOn * rtb_Integrator *
      realtimemodel_P.feedback.robustKi;

    /* Integrator: '<S2>/Integrator' */
    rtb_Integrator = realtimemodel_X.Integrator_CSTATE;
    if (tmp_0) {
      for (i = 0; i <= 4; i += 2) {
        /* Gain: '<S2>/State feedforward gain(Nx)' */
        _mm_storeu_pd(&realtimemodel_B.StatefeedforwardgainNx[i], _mm_mul_pd
                      (_mm_loadu_pd(&realtimemodel_P.feedback.Nx[i]),
                       _mm_set1_pd(realtimemodel_B.deg2rad)));
      }
    }

    /* StateSpace: '<S2>/State-Space' */
    _mm_storeu_pd(&rtb_xQ[0], _mm_add_pd(_mm_mul_pd(_mm_set_pd
      (realtimemodel_X.StateSpace_CSTATE[0], realtimemodel_P.StateSpace_C[0]),
      _mm_set_pd(realtimemodel_P.StateSpace_C[1],
                 realtimemodel_X.StateSpace_CSTATE[0])), _mm_mul_pd(_mm_set1_pd
      (realtimemodel_X.StateSpace_CSTATE[1]), _mm_loadu_pd
      (&realtimemodel_P.StateSpace_C[2]))));

    /* Sum: '<S2>/Sum' */
    tmp_3 = _mm_sub_pd(_mm_loadu_pd(&realtimemodel_B.StatefeedforwardgainNx[0]),
                       _mm_loadu_pd(&realtimemodel_B.x[0]));
    _mm_storeu_pd(&tmp_1[0], tmp_3);
    tmp_3 = _mm_sub_pd(_mm_loadu_pd(&realtimemodel_B.StatefeedforwardgainNx[2]),
                       _mm_loadu_pd(&realtimemodel_B.x[2]));
    _mm_storeu_pd(&tmp_1[2], tmp_3);
    tmp_3 = _mm_sub_pd(_mm_loadu_pd(&realtimemodel_B.StatefeedforwardgainNx[4]),
                       _mm_loadu_pd(&rtb_xQ[0]));
    _mm_storeu_pd(&tmp_1[4], tmp_3);

    /* Gain: '<S2>/State feedback gain' */
    tmp_2 = 0.0;
    for (i = 0; i < 6; i++) {
      tmp_2 += realtimemodel_P.feedback.K[i] * tmp_1[i];
    }

    /* Sum: '<S2>/Sum1' incorporates:
     *  Gain: '<S2>/State feedback gain'
     */
    rtb_Integrator += realtimemodel_B.InputfeedforwardgainNu + tmp_2;
    ...
    if (tmp) {
      srUpdateBC(realtimemodel_DW.FeedbackControllers_SubsysRanBC);
    }
  }
...
void realtimemodel_derivatives(void)
{
  ...
  /* Derivatives for TransferFcn: '<S3>/Real derivative' */
  _mm_storeu_pd(&tmp_0[0], _mm_add_pd(_mm_set_pd(_rtXdot->Realderivative_CSTATE
    [0], realtimemodel_X.Realderivative_CSTATE[0]), _mm_set_pd
    (realtimemodel_B.th_hubdeg, _rtXdot->Realderivative_CSTATE[1])));
  _rtXdot->Realderivative_CSTATE[1] = tmp_0[0];
  _rtXdot->Realderivative_CSTATE[0] = tmp_0[1];

  /* Derivatives for TransferFcn: '<S3>/Real derivative1' */
  _mm_storeu_pd(&tmp_0[0], _mm_add_pd(_mm_set_pd(_rtXdot->
    Realderivative1_CSTATE[0], realtimemodel_X.Realderivative1_CSTATE[0]),
    _mm_set_pd(realtimemodel_B.Sum1, _rtXdot->Realderivative1_CSTATE[1])));
  _rtXdot->Realderivative1_CSTATE[1] = tmp_0[0];
  _rtXdot->Realderivative1_CSTATE[0] = tmp_0[1];

  /* Derivatives for Enabled SubSystem: '<Root>/Feedback Controllers' */
  if (realtimemodel_DW.FeedbackControllers_MODE) {
    ...
    StateSpace_CSTATE = realtimemodel_X.StateSpace_CSTATE[0];
    StateSpace_CSTATE_0 = realtimemodel_X.StateSpace_CSTATE[1];
    x = realtimemodel_B.x[0];
    x_0 = realtimemodel_B.x[1];
    x_1 = realtimemodel_B.x[2];
    x_2 = realtimemodel_B.x[3];
    for (ri = 0; ri <= 0; ri += 2) {
      tmp = _mm_loadu_pd(&_rtXdot->StateSpace_CSTATE[ri]);
      _mm_storeu_pd(&_rtXdot->StateSpace_CSTATE[ri], _mm_add_pd(_mm_add_pd
        (_mm_add_pd(_mm_add_pd(_mm_add_pd(_mm_add_pd(_mm_mul_pd(_mm_loadu_pd
        (&realtimemodel_P.LQR.AQ[ri]), _mm_set1_pd(StateSpace_CSTATE)), tmp),
        _mm_mul_pd(_mm_loadu_pd(&realtimemodel_P.LQR.AQ[ri + 2]), _mm_set1_pd
                   (StateSpace_CSTATE_0))), _mm_mul_pd(_mm_loadu_pd
        (&realtimemodel_P.LQR.BQ[ri]), _mm_set1_pd(x))), _mm_mul_pd(_mm_loadu_pd
        (&realtimemodel_P.LQR.BQ[ri + 2]), _mm_set1_pd(x_0))), _mm_mul_pd
         (_mm_loadu_pd(&realtimemodel_P.LQR.BQ[ri + 4]), _mm_set1_pd(x_1))),
        _mm_mul_pd(_mm_loadu_pd(&realtimemodel_P.LQR.BQ[ri + 6]), _mm_set1_pd
                   (x_2))));
    }
  ...
  } else {
    {
      real_T *dx;
      int_T i1;
      dx = &(((XDot_realtimemodel_T *) realtimemodel_M->derivs)
             ->Integrator_CSTATE);
      for (i1=0; i1 < 3; i1++) {
        dx[i1] = 0.0;
      }
  ...
  /* End of Derivatives for SubSystem: '<Root>/Feedback Controllers' */
}
...
void realtimemodel_initialize(void)
{
  ...
  /* Start for Enabled SubSystem: '<Root>/Feedback Controllers' */
  realtimemodel_DW.FeedbackControllers_MODE = false;
  (void) memset(&(realtimemodel_XDis.Integrator_CSTATE), 1,
                3*sizeof(boolean_T));

  /* End of Start for SubSystem: '<Root>/Feedback Controllers' */
  ...
  /* SystemInitialize for Enabled SubSystem: '<Root>/Feedback Controllers' */
  /* InitializeConditions for Integrator: '<S2>/Integrator' */
  realtimemodel_X.Integrator_CSTATE = realtimemodel_P.Integrator_IC;

  /* End of SystemInitialize for SubSystem: '<Root>/Feedback Controllers' */
  ...
  realtimemodel_X.StateSpace_CSTATE[0] =
  ...
  /* SystemInitialize for Enabled SubSystem: '<Root>/Feedback Controllers' */
  /* InitializeConditions for StateSpace: '<S2>/State-Space' */
  realtimemodel_X.StateSpace_CSTATE[1] =
    realtimemodel_P.StateSpace_InitialCondition;
...
RT_MODEL_realtimemodel_T *realtimemodel(void)
{
  ...
  /* External mode info */
  realtimemodel_M->Sizes.checksums[0] = (3553430151U);
  realtimemodel_M->Sizes.checksums[1] = (286804286U);
  realtimemodel_M->Sizes.checksums[2] = (3797154480U);
  realtimemodel_M->Sizes.checksums[3] = (1016563170U);
  ...
  /* Initialize Sizes */
  realtimemodel_M->Sizes.numContStates = (7);/* Number of continuous states */
  ...
  realtimemodel_M->Sizes.numBlocks = (35);/* Number of blocks */
  realtimemodel_M->Sizes.numBlockIO = (14);/* Number of block outputs */
  realtimemodel_M->Sizes.numBlockPrms = (48);/* Sum of parameter "widths" */
