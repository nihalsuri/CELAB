...
MAKECMD         = "C:/PROGRA~1/MATLAB/R2024b/bin/win64/gmake"
HOST            = ANY
BUILD           = yes
...
MODEL                := realtimemodel
MODULES              := realtimemodel_data.c realtimemodel_tgtconn.c rt_nonfinite.c
MAKEFILE             := realtimemodel.mk
...
SLDRTDIR             := C:/PROGRA~1/MATLAB/R2024b/toolbox/sldrt
TARGETARCH           := win64
CC_LISTING           := 0
REBUILD_ALL          := 0

# if MAKEFILE is not defined, default to this makefile
ifeq ($(MAKEFILE),)
...
MODELLIB                  := 
MODELREF_LINK_LIBS        := 
MODELREF_LINK_RSPFILE     := realtimemodel_ref.rsp
...
# additional includes
REQ_INCLUDES += ;$(WQ)Z:/windows_profile/Desktop/CELAB-master/CELAB-master/CELAB3/24_StateSpace_LQR/2.4.3&2.4.7 NewCostfunction&Robust$(WQ);$(WQ)Z:/windows_profile/Desktop/CELAB-master/CELAB-master/CELAB3/24_StateSpace_LQR/2.4.3&2.4.7 NewCostfunction&Robust/realtimemodel_sldrt_win64$(WQ);$(WQ)$(MATLAB_ROOT)/extern/include$(WQ);$(WQ)$(MATLAB_ROOT)/simulink/include$(WQ);$(WQ)$(MATLAB_ROOT)/rtw/c/src$(WQ);$(WQ)$(MATLAB_ROOT)/rtw/c/src/ext_mode/common$(WQ);$(WQ)$(MATLAB_ROOT)/toolbox/coder/rtiostream/src$(WQ)
...
# C++ language standard
CLANG_CPPSTD := -std=c++$(if $(filter-out ISO_C++,$(filter ISO_C++%,$(TGT_FCN_LIB))),$(subst ISO_C++,,$(TGT_FCN_LIB)),03)
...
ALLLIBS := $(LIBS) $(S_FUNCTIONS_LIB) $(SLDRTDIR)/lib/$(TARGETARCH)/sldrtlib.a

# rules for linking the executable or modelref static library
ifeq ($(MODELREF_TARGET_TYPE),NONE)

.INTERMEDIATE : $(notdir $(PRODUCT)).tmp
$(notdir $(PRODUCT)).tmp : $(OBJS) $(MODELREF_LINK_LIBS) $(ALLLIBS)
	$(PRINTMSG)Linking $(basename $@)
	$(LLD) -shared $(if $(filter 1,$(CC_LISTING)),--Map=$(basename $@).map) -o $@ $(OBJS) @$(MODELREF_LINK_RSPFILE) $(ALLLIBS)

$(PRODUCT) : $(notdir $(PRODUCT)).tmp
ifeq ($(CC_LISTING),1)
	$(OBJDUMP) $< >$(@F).lst
endif
	$(OBJSTRIP) -s -o $@ $<
	$(PRINTMSG)Created Simulink Desktop Real-Time module $(@F)
