...
void realtime44_output(void)
{
  ...
  /* S-Function Block: <S4>/Encoder Input */
  {
    ...
    RTBIO_DriverIO(0, ENCODERINPUT, IOREAD, 1,
                   &realtime44_P.EncoderInput_Channels, &rtb_Integrator, &parm);
  }
  ...
  /* Gain: '<Root>/pulse2deg' */
  realtime44_B.pulse2deg = realtime44_P.sens.enc.pulse2deg * rtb_Integrator;
  tmp_0 = (rtmIsMajorTimeStep(realtime44_M) &&
           realtime44_M->Timing.TaskCounters.TID[2] == 0);
  if (tmp_0) {
    /* UnitDelay: '<S5>/Output' */
    rtb_Output_a = realtime44_DW.Output_DSTATE;
     ...
     *  UnitDelay: '<S5>/Output'
     */
    realtime44_B.Out = realtime44_P.sIn.position[realtime44_DW.Output_DSTATE];
  }
  ...
  if (tmp_0) {
     ...
     */
    rtb_Output_a += realtime44_P.FixPtConstant_Value;

    /* Switch: '<S7>/FixPt Switch' */
    if (rtb_Output_a > realtime44_P.WrapToZero_Threshold) {
      ...
      /* Switch: '<S7>/FixPt Switch' */
      realtime44_B.FixPtSwitch = rtb_Output_a;
    }
  ...
  rtb_Integrator = realtime44_P.filt.high.num[0] / realtime44_P.filt.high.den[0]
  ...
  rtb_degs2rpm = (realtime44_P.filt.high.num[1] / realtime44_P.filt.high.den[0] *
                  ...
                  rtb_Integrator) * realtime44_P.degs2rpm;

  /* Integrator: '<S3>/Integrator' */
  rtb_Integrator = realtime44_X.Integrator_CSTATE;

  /* Gain: '<S3>/deg2rad2' */
  rtb_deg2rad2 = realtime44_P.deg2rad * realtime44_B.pulse2deg;

  /* Gain: '<S3>/Integrator feedback' incorporates:
   *  Sum: '<S3>/Sum2'
   */
  realtime44_B.Integratorfeedback = (realtime44_B.deg2rad - rtb_deg2rad2) *
    realtime44_P.feedback.integ.acker.Ki;
  if (tmp_0) {
   ...
   *  Sum: '<S3>/Sum'
   */
  rtb_degs2rpm = (((realtime44_B.StatefeedforwardgainNx[1] -
                    realtime44_P.rpm2rads * rtb_degs2rpm) *
                   realtime44_P.feedback.integ.acker.K[1] +
                   (realtime44_B.StatefeedforwardgainNx[0] - rtb_deg2rad2) *
                   realtime44_P.feedback.integ.acker.K[0]) +
                  realtime44_B.InputfeedforwardgainNu) + rtb_Integrator;

  /* Saturate: '<S3>/Saturation' */
  if (rtb_degs2rpm > realtime44_P.Saturation_UpperSat) {
    rtb_degs2rpm = realtime44_P.Saturation_UpperSat;
  } else if (rtb_degs2rpm < realtime44_P.Saturation_LowerSat) {
...
void realtime44_derivatives(void)
{
  ...
  _rtXdot->Highpassfilterrealderivative_CSTATE[1] =
    realtime44_X.Highpassfilterrealderivative_CSTATE[0];
  _rtXdot->Highpassfilterrealderivative_CSTATE[0] += realtime44_B.pulse2deg;

  /* Derivatives for Integrator: '<S3>/Integrator' */
  _rtXdot->Integrator_CSTATE = realtime44_B.Integratorfeedback;
}
...
RT_MODEL_realtime44_T *realtime44(void)
{
  ...
  rtmSetTFinal(realtime44_M, 34.0);
  realtime44_M->Timing.stepSize0 = 0.001;
  realtime44_M->Timing.stepSize1 = 0.001;
  realtime44_M->Timing.stepSize2 = 5.0;

  /* External mode info */
  realtime44_M->Sizes.checksums[0] = (328665852U);
  realtime44_M->Sizes.checksums[1] = (849020202U);
  realtime44_M->Sizes.checksums[2] = (842359507U);
  realtime44_M->Sizes.checksums[3] = (1649566909U);
  ...
  realtime44_M->Sizes.numBlocks = (28);/* Number of blocks */
  realtime44_M->Sizes.numBlockIO = (7);/* Number of block outputs */
  realtime44_M->Sizes.numBlockPrms = (31);/* Sum of parameter "widths" */
  return realtime44_M;
}
